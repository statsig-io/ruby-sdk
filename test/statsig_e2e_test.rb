...
class StatsigE2ETest < Minitest::Test
  ...
  def test_layer
    ...
    layer = driver.get_layer(@statsig_user, 'a_layer')
    assert(layer.get('experiment_param', 'ERR') == 'test')
    assert(layer.get('layer_param', false) == true)
    assert(layer.get('second_layer_param', false) == true)

    layer = driver.get_layer(@random_user, 'b_layer_no_alloc')
    assert(layer.get('b_param', 'ERR') == 'layer_default')

    layer = driver.get_layer(@random_user, 'c_layer_with_holdout')
    assert(layer.get('holdout_layer_param', 'ERR') == 'layer_default')
